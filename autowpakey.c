...
int main (void) {
	...
	static wchar_t reasonstr[65537];
	static WLANKEY key[2];

	status = DecodeWLANKeys(&keys, &numkeys);
	if (status != ERROR_SUCCESS){
		fwprintf (stderr, L"Error decoding WLAN keys\n");
	...
	if (status != ERROR_SUCCESS){
		fwprintf (stderr, L"Error getting local SSID\n");
		fwprintf (stderr, L"Status: 0x%08X\n", status);
		if (reason != 0){
	...
	if (index >= 0){
		...
		fwprintf (stderr, L"No matching SSID available\n");
		return 1;
	}
}
